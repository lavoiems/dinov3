...
def text_global_pool(
    ...
    if pool_type == 'first':
        ...
        pooled = x[torch.arange(x.shape[0], device=x.device), text.argmax(dim=-1).argmax(dim=-1)]
    elif pool_type == 'eos':
...
class TextTransformer(nn.Module):
    ...
    def _embeds(self, text) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        B, seq_len, V = text.shape

        # x = F.one_hot(text, self.vocab_size).to(text.device).float()
        x = (text @ self.token_embedding.weight).to(cast_dtype)
